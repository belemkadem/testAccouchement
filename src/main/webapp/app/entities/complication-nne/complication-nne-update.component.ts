...
@Component({
    selector: 'jhi-complication-nne-update',
    templateUrl: './complication-nne-update.component.html'
})
export class ComplicationNNEUpdateComponent implements OnInit {
    ...
    save() {
        this.isSaving = true;
        if (this.complicationNNE.id !== undefined) {
            this.subscribeToSaveResponse(this.complicationNNEService.update(this.complicationNNE));
        } else {
            this.subscribeToSaveResponse(this.complicationNNEService.create(this.complicationNNE));
        }
    }

    protected subscribeToSaveResponse(result: Observable<HttpResponse<IComplicationNNE>>) {
        result.subscribe((res: HttpResponse<IComplicationNNE>) => this.onSaveSuccess(), (res: HttpErrorResponse) => this.onSaveError());
    }
